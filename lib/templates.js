...
// 自动导入 @/views/**/page.js
const requireContext = require.context('@/views', true, /page\\.js$/);

requireContext.keys().forEach((key) => {
  const routeConfig = requireContext(key).default;

  if (!routeConfig) {
    console.warn(\`[Router] 空模块: \${key}\`);
    return;
  }

  if (Array.isArray(routeConfig)) {
    routes.push(...routeConfig);
  } else if (routeConfig.path && routeConfig.component) {
    routes.push(routeConfig);
  } else {
    console.warn(\`[Router] 无效路由配置: \${key}\`, routeConfig);
  }
});

console.log('✅ 已注册路由:', routes);
